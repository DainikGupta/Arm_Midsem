        ...
        ENTRY
__main    FUNCTION
		VLDR.F32 S0, =100	;VALUE 0F X.
		VLDR.F32 S1, =2		;VALUE OF RADIUS(r).
...
CREATE_CIRCLE	VADD.F32 S0, S0, S7
				VCMP.F32 S0, S9			
...
CALC_X_Y	VMOV.F32 S2, #1
			VMOV.F32 S3, #1
			VMUL.F32 S2, S1, S6		;X=rCOS(Q)
			VADD.F32 S2, S2, S11	;X=A+RCOS(Q)
			VMUL.F32 S3, S1, S8		;Y=rSIN(Q)
			VADD.F32 S3, S3, S12	;Y=B+RSIN(Q)
			MOV R10, #-1
			;B CREATE_CIRCLE
